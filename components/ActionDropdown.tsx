...
import { deletFile, renameFile, updateFileUsers } from '@/lib/actions/file.actions';
...
const ActionDropdown = ({ file }: { file: Models.Document }) => {
	...
	const handleAction = async () => {
		...
		const actions = {
			...
			delete: () => deletFile({ fileId: file.$id, path, bucketFileId: file.bucketFileId }),
		};
		...
		return (
			<DialogContent className=" shad-dialog button">
				<DialogHeader className=" flex flex-col gap-3">
					...
					{value === 'share' && <ShareInput file={file} onInputChange={setEmails} onRemove={handleRemoveUsers} />}
					{value === 'delete' && (
				...
				{['rename', 'share', 'delete'].includes(value) && (
					<DialogFooter className=" flex flex-col gap-3">
						...
						<Button onClick={handleAction} className=" modal-submit-button" type="submit">
